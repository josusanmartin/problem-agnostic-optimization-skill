...
def render_svg(points: list[Point], output: Path, title: str, ylabel: str, direction: str) -> None:
    ...
    if token_values:
        token_min, token_max = nice_range(token_values)
    else:
        token_min, token_max = 0.0, 1.0
    ...
    parts = [
        ...
        "<style>text{font-family:Arial,Helvetica,sans-serif;font-size:13px}.small{font-size:11px}.title{font-size:18px}</style>",
    ...
    for value in ticks(score_min, score_max):
        yy = y_score(value)
        parts.append(f'<line x1="{LEFT}" y1="{yy:.2f}" x2="{WIDTH - RIGHT}" y2="{yy:.2f}" stroke="#eeeeee"/>')
    ...
    for i in range(0, len(points), max(1, len(points) // 10)):
        xx = x(i)
        parts.append(f'<line x1="{xx:.2f}" y1="{TOP}" x2="{xx:.2f}" y2="{HEIGHT - BOTTOM}" stroke="#f3f3f3"/>')
    ...
    for value in ticks(token_min, token_max):
        yy = y_token(value)
        parts.append(f'<text x="{WIDTH - RIGHT + 10}" y="{yy + 4:.2f}" fill="#2563eb" class="small">{value / 1000:.3g}k</text>')

    parts.extend(
        [
            f'<line x1="{LEFT}" y1="{TOP}" x2="{LEFT}" y2="{HEIGHT - BOTTOM}" stroke="#222"/>',
            f'<line x1="{WIDTH - RIGHT}" y1="{TOP}" x2="{WIDTH - RIGHT}" y2="{HEIGHT - BOTTOM}" stroke="#2563eb"/>',
            f'<line x1="{LEFT}" y1="{HEIGHT - BOTTOM}" x2="{WIDTH - RIGHT}" y2="{HEIGHT - BOTTOM}" stroke="#222"/>',
            f'<text x="{WIDTH / 2:.1f}" y="{HEIGHT - 22}" text-anchor="middle">Candidate #</text>',
            f'<text x="22" y="{HEIGHT / 2:.1f}" transform="rotate(-90 22 {HEIGHT / 2:.1f})" text-anchor="middle">{escape(ylabel)}</text>',
            f'<text x="{WIDTH - 24}" y="{HEIGHT / 2:.1f}" transform="rotate(90 {WIDTH - 24} {HEIGHT / 2:.1f})" text-anchor="middle" fill="#2563eb">Cumulative tokens</text>',
            best_svg,
            token_svg,
        ]
    )
    ...
    for p in points:
        ...
        opacity = "0.95" if color != "#cfcfcf" else "0.55"
        parts.append(f'<circle cx="{xx:.2f}" cy="{yy:.2f}" r="4" fill="{color}" opacity="{opacity}"/>')
        if p.decision in BEST_DECISIONS and p.label:
            label = escape(p.label[:54])
            parts.append(f'<text x="{xx + 8:.2f}" y="{yy - 7:.2f}" fill="#169c55" class="small" transform="rotate(-25 {xx + 8:.2f} {yy - 7:.2f})">{label}</text>')

    legend_x = WIDTH - RIGHT - 160
    legend_y = TOP + 8
    parts.extend(
        [
            f'<rect x="{legend_x}" y="{legend_y - 16}" width="150" height="72" fill="white" stroke="#dddddd"/>',
            f'<circle cx="{legend_x + 14}" cy="{legend_y}" r="4" fill="#cfcfcf" opacity="0.55"/><text x="{legend_x + 26}" y="{legend_y + 4}" class="small">Discarded</text>',
            f'<circle cx="{legend_x + 14}" cy="{legend_y + 20}" r="4" fill="#169c55"/><text x="{legend_x + 26}" y="{legend_y + 24}" class="small">Promoted/kept</text>',
            f'<line x1="{legend_x + 7}" y1="{legend_y + 40}" x2="{legend_x + 21}" y2="{legend_y + 40}" stroke="#169c55" stroke-width="2"/><text x="{legend_x + 26}" y="{legend_y + 44}" class="small">Running best</text>',
            f'<line x1="{legend_x + 7}" y1="{legend_y + 60}" x2="{legend_x + 21}" y2="{legend_y + 60}" stroke="#2563eb" stroke-width="2"/><text x="{legend_x + 26}" y="{legend_y + 64}" class="small">Tokens</text>',
        ]
    )
